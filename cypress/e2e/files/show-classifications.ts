...
Then(/^the PDF shows all of its languages$/, () => {
  const languages = new Set([...(pdf.pageClassifications?.flatMap((it) => it.languages) ?? [])]);
...
Then(/^the PDF shows all of its page categories$/, () => {
  const categories = new Set([...(pdf.pageClassifications?.flatMap((it) => it.categories) ?? [])]);
...
When(/^the PDF's table of contents is toggled$/, () => {
  ...
  // Scroll to bottom again as the toc will have appeared partially out of frame.
  cy.get('@detail').find('.asset-detail-scroll-container').scrollTo('bottom');
});
...
const checkTableOfContentsHasCategory = (category: PageCategory): void => {
  if (pdf.pageClassifications === null) {
    return;
  }

  const classifications = pdf.pageClassifications.filter((it) => it.categories.includes(category));
