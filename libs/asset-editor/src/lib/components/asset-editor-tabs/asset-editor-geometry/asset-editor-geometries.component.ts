...
interface StudyToDisplay extends Study {
  displayTitle: string;
}

@UntilDestroy()
@Component({
  selector: 'asset-sg-edit-geometries',
  styleUrls: ['./asset-editor-geometries.component.scss'],
  templateUrl: './asset-editor-geometries.component.html',
  standalone: false,
...
export class AssetEditorGeometriesComponent implements OnInit {
  ...
  public geometrySelected(event: StudyToDisplay[]) {
    this.selectedStudy = this.studies.find((study) => study.studyId === event[0].studyId);
  }
  ...
  ngOnInit() {
    this.form.controls.studies.valueChanges.pipe(startWith(this.form.controls.studies.value)).subscribe((value) => {
      this._state.set({ studies: value });
    });
    // const studies = this.form.controls.studies.value;

    this._studies$.pipe(untilDestroyed(this)).subscribe((studies) => {
      if (!eqStudies.equals(studies, this.form.getRawValue().studies)) {
        this.form.patchValue({ studies });
