...
import {
  ...
  LocalDate,
  LocalizedItemCode,
  mapGeometryTypeToStudyTag,
  mapGeometryTypeToStudyType,
  mapStudyTagToGeometryType,
  UpdateAssetFileData,
  UpdateGeometryData,
  Workflow,
        ...
        studyMapping.set(geometry.id, {
          ...
          geom:
            geometry.type === GeometryType.Point
              ...
              : {
                  _tag: mapGeometryTypeToStudyTag(geometry.type) as 'LineString' | 'Polygon',
      ...
      for (const study of studies) {
        if (study.studyId.includes('_new_')) {
          newGeometries.push({
            ...
            type: mapStudyTagToGeometryType(study.geom._tag),
