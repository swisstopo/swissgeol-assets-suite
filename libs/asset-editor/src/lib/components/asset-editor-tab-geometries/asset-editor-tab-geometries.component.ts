...
export class AssetEditorTabGeometriesComponent implements OnInit {
    ...
    _ngAfterViewInit(): void {
        ...
        fromEventPattern<SelectEvent>(
            ...
            .subscribe(([e, selectedStudyId]) => {
                ...
                if (O.isSome(featureId)) {
                    if (
                        O.isNone(selectedStudyId) ||
                        (O.isSome(selectedStudyId) && featureId.value !== selectedStudyId.value)
                    ) {
        ...
        fromEventPattern<TranslateEvent>(
            ...
            .subscribe(() => {
                const headFeature = pipe(
                    this._selectInteraction.getFeatures().getArray(),
                    NEA.fromArray,
                    O.map(NEA.head),
                );
        ...
        selectedStudy$
            .pipe(
                OO.fromFilteredSome,
                distinctUntilChanged(
                    contramap((a: { studies: Studies; selectedStudy: Study }) => a.selectedStudy)(eqStudyByStudyId)
                        .equals,
                ),
                untilDestroyed(this),
            )
                        ...
                        if (O.isSome(id)) {
                            if (id.value.startsWith(makeCoordFeatureIdStartWith(selectedStudy))) {
                                if (O.isSome(_currentStudyCoordIndex)) {
                                    if (
                                        f.getId() === makeCoordFeatureId(selectedStudy, _currentStudyCoordIndex.value)
                                    ) {
    ...
    drawNewGeometry() {
        this._state.set(
            flow(
                updateMode('choose-new-geometry'),
                updateSelectedStudyId(O.none),
                    ...
                    Polygon: a => {
                        ...
                        if (this._state.get().dragMode !== 'coord') {
                            a.coords.forEach((coord, i) => {
                                ...
                                const marker = this._vectorSourceAssetGeoms.getFeatureById(
                                    makeCoordFeatureId(study, i),
                                );
                                if (marker) {
                                    marker.setGeometry(new Point(olCoordsFromLV95Array([coord])[0]));
                                    marker.setStyle(featureStyles.bigPointStyleAssetSelected);
                ...
                const updateResult = pipe(
                    ...
                    O.chain(studyId =>
                        pipe(
                            ...
                            O.bindTo('index'),
                            O.bind('study', ({ index }) =>
                                pipe(studies, A.lookup(index), O.chain(getStudyWithGeomWithCoords)),
                            ),
                ...
                if (!O.getEq(eqNumber).equals(O.some(currentPointIndex), this._state.get().currentStudyCoordIndex)) {
                        ...
                        if (currentPointIndex > 0) {
                            ...
                            const feature1 = decorateFeature(
                                new Feature({
                                    geometry: new LineString([
                                        olCoordsFromLV95Array([
                                            updateResult.value.updatedStudy.geom.coords[currentPointIndex - 1],
                                        ])[0],
                                        ev.coordinate,
                                    ]),
                                }),
                    ...
                    if (preview1LineFeature) {
                        ...
                        if (preview1LineGeometry) {
                            preview1LineGeometry.setCoordinates([
                                preview1LineGeometry.getCoordinates()[0],
                                ev.coordinate,
                            ]);
                        }
                    }
                    if (geometryType === 'Polygon') {
                        ...
                        if (preview2LineFeature) {
                            ...
                            if (preview2LineGeometry) {
                                preview2LineGeometry.setCoordinates([
                                    preview2LineGeometry.getCoordinates()[0],
                                    ev.coordinate,
                                ]);
                            }
                        }
...
const setFeatureStyle = (feature: Feature, study: Study, selected: boolean) => {
    const style = Geom.matchStrict({
        ...
        LineString: () =>
            selected ? featureStyles.lineStringStyleAsset : featureStyles.lineStringStyleAssetNotSelected,
    })(study.geom);
                                        ...
                                        Polygon: a => {
                                            ...
                                            return currentStudyCoordIndex !== 0 || a.coords.length === 1
                                                ...
                                                : pipe(
                                                      maybeGeom,
                                                      O.chain(g =>
                                                          updateStudyCoords(g, g.coords.length - 1, validatedValue),
                                                      ),
                                                  );
                                        },
                                    })(study.geom),
