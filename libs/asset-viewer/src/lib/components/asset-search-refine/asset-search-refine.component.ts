...
export class AssetSearchRefineComponent implements OnInit, OnDestroy, AfterViewInit {
  ...
  public ngAfterViewInit() {
    ...
    this.subscriptions.add(this.minDateControl.valueChanges.pipe(
      ).subscribe(value => {
    ...
    this.subscriptions.add(this.maxDateControl.valueChanges.pipe(
      ).subscribe(value => {
        if (value instanceof Date || value === undefined) {
  ...
  public updateManCatLabel(item: AvailableItem) {
    const availableItem: AvailableItem | undefined = this.availableManCatLabels.find((element) => element.item.code === item.item.code);
    if (availableItem !== undefined) {
      availableItem.isActive = !item.isActive;
    }
    this.updateSearch({
      manCatLabelItemCodes: this
        .availableManCatLabels.filter((element) => element.isActive).map((element) => element.item.code),
    });
  }

  public updateAssetKindItems(item: AvailableItem) {
    const availableItem: AvailableItem | undefined = this.availableAssetKindItems.find((element) => element.item.code === item.item.code);
    if (availableItem !== undefined) {
      availableItem.isActive = !item.isActive;
    }
    this.updateSearch({
      assetKindItemCodes: this
        .availableAssetKindItems.filter((element) => element.isActive).map((element) => element.item.code),
    });
  }

  public updateLanguages(item: AvailableItem) {
    const availableItem: AvailableItem | undefined = this.availableLanguages.find((element) => element.item.code === item.item.code);
    if (availableItem !== undefined) {
      availableItem.isActive = !item.isActive;
    }
    this.updateSearch({
      languageItemCodes: this
        .availableLanguages.filter((element) => element.isActive).map((element) => element.item.code),
    });
  }

  public updateUsageCodes(item: AvailableValueCount<UsageCode>) {
    const availableItem: AvailableValueCount<UsageCode> | undefined = this.usageCodes.find((element) => element.value === item.value);
    if (availableItem !== undefined) {
      availableItem.isActive = !item.isActive;
    }
    this.updateSearch({
      usageCodes: this
        .usageCodes.filter((element) => element.isActive).map((element) => element.value),
    });
  }

  public updateGeometryCodes(item: AvailableValueCount<GeometryCode | 'None'>) {
    const availableItem: AvailableValueCount<GeometryCode | 'None'> | undefined = this.availableGeomteryCodes.find((element) => element.value === item.value);
    if (availableItem !== undefined) {
      availableItem.isActive = !item.isActive;
    }
    this.updateSearch({
      geomCodes: this
        .availableGeomteryCodes.filter((element) => element.isActive).map((element) => element.value),
    });
  }
  ...
  private initSubscriptions() {
    ...
    this.subscriptions.add(this.createDateRange$.subscribe(dateRange => {
      this.createDateRange = dateRange;
    }));

    this.subscriptions.add(this.availableAuthors$.subscribe(authors => {
      this.availableAuthors = authors ?? [];
      this.filteredAuthors = authors ?? [];
    }));

    this.subscriptions.add(this.usageCodes$.subscribe(usageCodes => {
      this.usageCodes = usageCodes ?? [];
    }));

    this.subscriptions.add(this.availableGeometryCodes$.subscribe(geomCode => {
      this.availableGeomteryCodes = geomCode ?? [];
    }));

    this.subscriptions.add(this.availableManCatLabels$.pipe(concatLatestFrom(() => this._currentLang$),
      map(([manCatLabels, lang]) => {
        this.availableManCatLabels = this.setLanguageForFilter(lang, manCatLabels ?? []);
      })).subscribe());

    this.subscriptions.add(this.availableLanguages$.pipe(concatLatestFrom(() => this._currentLang$),
      map(([languages, currentLang]) => {
        this.availableLanguages = this.setLanguageForFilter(currentLang, languages ?? []);
      })).subscribe());

    this.subscriptions.add(this.availableAssetKindItems$.pipe(concatLatestFrom(() => this._currentLang$),
      map(([assetKindItems, currentLang]) => {
        this.availableAssetKindItems = this.setLanguageForFilter(currentLang, assetKindItems ?? []);
      })).subscribe());

    this.subscriptions.add(this.authorAutoCompleteControl.valueChanges.pipe(
      startWith(''),
    ...
    this.subscriptions.add(this._currentLang$.subscribe((lang) => {
