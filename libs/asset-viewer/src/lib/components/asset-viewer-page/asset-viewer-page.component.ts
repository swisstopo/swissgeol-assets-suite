...
export class AssetViewerPageComponent implements AfterViewInit {
    ...
    public currentAssetId$ = this._store.select(selectCurrentAssetDetail).pipe(map((currentAsset) => currentAsset?.assetId), map(O.fromNullable));
    ...
    constructor() {
        ...
        merge(
            ...
            this.polygonChanged$.pipe(map(polygon => actions.searchByFilterConfiguration({ filterConfiguration: { polygon } }))),
