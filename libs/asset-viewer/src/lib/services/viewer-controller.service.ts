...
export class ViewerControllerService {
  ...
  private readonly allStudyService = inject(GeometryService);
  ...
  private async initializeFromStore(): Promise<void> {
    ...
    const studies = await firstValueFrom(this.store.select(selectGeometries));
    if (studies.length === 0) {
  ...
  private async loadStudies(): Promise<void> {
    ...
    const studies = await firstValueFrom(this.allStudyService.fetchAll());
    this.store.dispatch(actions.setGeometries({ geometries: studies, isLoading: false }));
  }
