...
export class ViewerParamsService {
  ...
  private subscribe(): void {
    ...
    // Ensure that the params stay up-to-date when something navigates to another page.
    // This is mainly here so that navigations to and from the favorites viewer work.
    this.subscription.add(
      this.router.events.subscribe(async (event) => {
        ...
        this.params.query = {
          ...this.params.query,
          favoritesOnly: this.parseFavoritesOnlyFromUrl(),
        };
        this.writeParamsToStore();
      })
    );
