...
import { fakeAssetPatch, fakeAssetUsage, fakeContact, fakeUser } from '../asset-edit/asset-edit.fake';
import { AssetEditDetail } from '../asset-edit/asset-edit.service';
import { PrismaService } from '../prisma/prisma.service';
import { AssetData, AssetRepo } from '../repos/asset.repo';
...
describe(AssetSearchService, () => {
  ...
  describe('search', () => {
    ...
    const testSearchInProperty = <T extends string | number>(
      text: T,
      setup: (asset: PatchAsset, text: T) => PatchAsset | Promise<PatchAsset>,
    ) => async () => {
      // Given
    ...
    it('finds text in titlePublic', testSearchInProperty(faker.string.uuid(), (asset, text) => ({
      ...asset,
      titlePublic: text,
    })));

    it('finds text in titleOriginal', testSearchInProperty(faker.string.uuid(), (asset, text) => ({
      ...asset,
      titleOriginal: text,
    })));

    const contactData = fakeContact();
    it('finds text in contactNames', testSearchInProperty(contactData.name, async (asset) => {
      const contact = await prisma.contact.create({ data: contactData })
      return {
        ...asset,
        assetContacts: [
          { contactId: contact.contactId, role: 'author' }
        ],
      }
    }));
    ...
    it('finds assets by minimum createDate', async () => {
      ...
      // When
      const result = await service.search({ createDate: {
    ...
    it('finds assets by usageCode', async () => {
      ...
      const asset = await create({ patch: {
          ...fakeAssetPatch(),
    ...
    it('finds assets by authorId', async () => {
      ...
      const asset = await create({ patch: { ...fakeAssetPatch(), assetContacts: [
            { contactId: contact1.contactId, role: 'author' },
          ]}, user: fakeUser() });
      await create({ patch: { ...fakeAssetPatch(), assetContacts: [
            { contactId: contact1.contactId, role: 'supplier' },
          ]}, user: fakeUser() });
      await create({ patch: { ...fakeAssetPatch(), assetContacts: [
            { contactId: contact2.contactId, role: 'author' },
          ]}, user: fakeUser() });
  ...
  describe('syncWithDatabase', () => {
    ...
    it('reports progress', async () => {
      ...
      for (let i = 0; i < 500; i++) {
        await Promise.all(Array.from({ length: 10 }, () => assetRepo.create({
