...
import { fakeAssetData, fakeContact, fakeUserData } from '@/features/assets/asset.fake';
...
describe(AssetSearchService, () => {
  ...
  const service = new AssetSearchService(elastic, prisma, assetRepo, geometryRepo);
  ...
  const create = async (data: CreateAssetData): Promise<Asset> => {
    const asset = await assetRepo.create(data);
    await service.register(asset);
    return asset;
  };

  const createItem = async (data: CreateAssetData): Promise<AssetSearchResultItem> => {
  ...
  describe('search', () => {
    ...
    const testSearchInProperty =
      <T extends string | number>(
        text: T,
        setup: (asset: CreateAssetData, text: T) => CreateAssetData | Promise<CreateAssetData>,
      ) =>
      async () => {
        // Given
        const user = await userRepo.create(fakeUserData());
        const data = await setup({ ...fakeAssetData(), creatorId: user.id }, text);
        const asset = await createItem(data);
        await createItem({ ...fakeAssetData(), creatorId: user.id });
    ...
    it('finds assets by minimum createdAt', async () => {
      // Given
      const user = await userRepo.create(fakeUserData());
      const asset = await createItem({ ...fakeAssetData(), creatorId: user.id });
      await createItem({
        ...fakeAssetData(),
    ...
    it('finds assets by kindCode', async () => {
      ...
      await createItem({ ...fakeAssetData(), kindCode: code2, creatorId: user.id });
      await createItem({ ...fakeAssetData(), kindCode: code3, creatorId: user.id });
    ...
    it('finds assets by topicCode', async () => {
      ...
      await createItem({ ...fakeAssetData(), topicCodes: [code2], creatorId: user.id });
      await createItem({ ...fakeAssetData(), topicCodes: [code3], creatorId: user.id });
  ...
  describe('aggregate', () => {
    ...
    it('aggregates stats for a single asset', async () => {
      // Given
      const user = await userRepo.create(fakeUserData());
      const asset = await create({ ...fakeAssetData(), creatorId: user.id });
