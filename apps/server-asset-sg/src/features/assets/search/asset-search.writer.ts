...
export class AssetSearchWriter {
  private readonly logger = new Logger(AssetSearchWriter.name);
  ...
  async write(oneOrMore: Asset | Asset[]): Promise<void> {
    ...
    const res = await this.elastic.bulk({
      index: this.options.index,
      refresh: this.options.shouldRefresh,
      operations,
    });
    if (res.errors) {
      this.logger.error('Failed to write to Elasticsearch', { errors: res.items });
      throw new Error('Failed to write to Elasticsearch');
    }
  }
