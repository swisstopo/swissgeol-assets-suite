...
import { fakeAssetData, fakeUserData } from './asset.fake';
...
describe(AssetRepo, () => {
  ...
  describe('list', () => {
    ...
    it('returns the specified amount of records', async () => {
      // Given
      const user = await userRepo.create(fakeUserData());
      const record1 = await repo.create({ ...fakeAssetData(), creatorId: user.id });
      const record2 = await repo.create({ ...fakeAssetData(), creatorId: user.id });
      const record3 = await repo.create({ ...fakeAssetData(), creatorId: user.id });
      await repo.create({ ...fakeAssetData(), creatorId: user.id });
      await repo.create({ ...fakeAssetData(), creatorId: user.id });
  ...
  describe('update', () => {
    ...
    it('should update the fields of an existing record', async () => {
      ...
      const record = await repo.create({ ...fakeAssetData(), creatorId: creator.id });
      const data = fakeAssetData();
