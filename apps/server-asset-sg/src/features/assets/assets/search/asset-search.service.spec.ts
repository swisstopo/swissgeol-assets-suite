...
describe(AssetSearchService, () => {
  ...
  const createItem = async (data: AssetEditData): Promise<AssetSearchResultItem> => {
  ...
  describe('search', () => {
    ...
    const testSearchInProperty =
      ...
      async () => {
        ...
        const asset = await createItem({ patch, user });
        await createItem({ patch: fakeAssetPatch(), user });
    ...
    it('finds assets by minimum createDate', async () => {
      // Given
      const user = await userRepo.create(fakeUserData());
      const asset = await createItem({ patch: fakeAssetPatch(), user });
      await createItem({
        patch: {
          ...fakeAssetPatch(),
    ...
    it('finds assets by assetKindItemCode', async () => {
      ...
      await createItem({ patch: { ...fakeAssetPatch(), assetKindItemCode: code2 }, user });
      await createItem({ patch: { ...fakeAssetPatch(), assetKindItemCode: code3 }, user });
    ...
    it('finds assets by manCatLabelItemCodes', async () => {
      ...
      await createItem({ patch: { ...fakeAssetPatch(), manCatLabelRefs: [code2] }, user });
      await createItem({ patch: { ...fakeAssetPatch(), manCatLabelRefs: [code3] }, user });
