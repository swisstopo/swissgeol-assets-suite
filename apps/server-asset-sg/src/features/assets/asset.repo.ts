...
export class AssetRepo implements Repo<Asset, AssetId, FullAssetData> {
  ...
  private async manageStudies(assetId: AssetId, data: (Study | StudyData)[]): Promise<void> {
    ...
    for (const [type, studies] of Object.entries(studiesToUpdate) as Array<[StudyType, Study[]]>) {
      await this.deleteStudies(assetId, type, studies.map((it) => it.id));
  ...
  private async deleteStudies(assetId: AssetId, type: StudyType, knownIds: StudyId[]): Promise<void> {
    const condition = knownIds.length === 0
      ? ''
      : Prisma.sql`AND study_${Prisma.raw(type)}_id NOT IN (${Prisma.join(knownIds, ',')})`;
  ...
  private async createStudies(assetId: AssetId, type: StudyType, data: StudyData[]): Promise<void> {
    ...
    const values = data.map((it) => Prisma.sql`
  ...
  private async updateStudies(assetId: AssetId, type: StudyType, data: Study[]): Promise<void> {
    ...
    // TODO does this work?
    const cases = data.map((it) => Prisma.sql`
...
const mapDataToPrisma = (data: FullAssetData) => satisfy<Partial<Prisma.AssetCreateInput & Prisma.AssetUpdateInput>>()({
...
const mapDataToPrismaCreate = (data: FullAssetData): Prisma.AssetCreateInput => ({
  ...
  isExtract: false,
  assetMain: data.links.parent == null ? undefined : {
    connect: {
...
const mapDataToPrismaUpdate = (id: AssetId, data: FullAssetData): Prisma.AssetUpdateInput => ({
  ...mapDataToPrisma(data),
  assetMain: data.links.parent == null
    ? { disconnect: true }
    : {
  ...
  siblingXAssets: {
    ...
    createMany: {
      data: data.links.siblings.filter((siblingId) => siblingId < id).map((siblingId) => ({
        assetYId: siblingId,
      })),
      skipDuplicates: true,
    },
