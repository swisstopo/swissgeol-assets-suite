...
export const importAssets = async () => {
    ...
    `)) as any[]
        ...
        .map(
            a =>
                `(${a.study_location_id}, ${a.asset_id}, '${
                    a.geom_quality_item_code
                }', st_geomfromtext('${a.geom.replace(/(\d+\.\d+)/g, a => Math.round(a * 1000) / 1000)}', 2056))`,
        )
        .join(',\n');
...
const makeImportValueList =
    ...
    async <T extends string>(
        ...
        if (doImportToTable) {
            await importToTable(
                tableName,
                buildPath(`wertelisten/${valueListFileName}`),
                createModelFromParsedCsv(false),
            );
        }

        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        return (
            (await readFromFileToModel(
                buildPath(`${path}/${valueListFileName}`),
                createModelFromParsedCsv(true),
            )) as any
        ).models;
    };
